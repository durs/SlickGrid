...
(function($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function getColContentSize(columnDef, $gridCanvas, isInit) {
      ...
      var i, ii, tempVal, maxLen, maxIndex;
      var maxColWidth = 0;
      var headerWidth = 0;
    ...
    function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
      ...
      //-----------------------------------------------------------------
      // append row atributes from getItemMetadata() call (appended by durs 2022-04-27)

      var rowTokens = [
        "<div class='ui-widget-content ", rowCss,
        "' style='top:", (getRowTop(row) - frozenRowOffset), "px' "
      ]
      var metadata_attrs = metadata && metadata.attrs;
      for (var rowAttr in metadata_attrs) {
        rowTokens.push(rowAttr + "='" + metadata_attrs[rowAttr] + "' ");
      }
      rowTokens.push(">");
      var rowHtml = rowTokens.join("");
    ...
    function applyFormatResultToCellNode(formatterResult, cellNode, suppressRemove) {
      ...
      //-----------------------------------------------------------------
      // check object and ignore result (appended by durs 2022-04-27)
      ...
      if (formatterResult.ignore) return;
    ...
    */
    ...
    //--------------------------------------------------------------------------------------
    // process touch events (only taps, pan and zoom commented, appended by durs 2022-04-27)
    ...
    var touch = {
      ...
      process_tap: function(e, ntap, ktap, tap) {
        var cell = getCellFromEvent(e);
        if (!cell || (currentEditor !== null && activeRow == cell.row && activeCell == cell.cell)) {
          return;
        }
        trigger(self.onTouchTap, { grid: self, row: cell.row, cell: cell.cell, ntap: ntap, ktap: ktap }, e);
      }
      // process_pan: function(p, tmout) {
      //     if (touch.p && !touch.t) touch.t = setTimeout(function() {
      //         touch.t = undefined;
      //         var ofsx = touch.p.left - p.left;
      //         var ofsy = touch.p.top - p.top;
      //         self.pan({ left: ofsx, top: ofsy });
      //         touch.p = p;
      //         if (touch.n > 0) {
      //             touch.n--;
      //             var p2 = { left: p.left - ofsx*2/3, top: p.top - ofsy*2/3}
      //             if (p2.left > 0 || p2.top > 0) touch.process_pan(p2, tmout);
      //         }
      //     }, tmout);
      // },
      // process_zoom: function(touches, tmout) {
      //     var ofs = self.offset();
    ...
    function handleTouchMove(evt) {
      ...
      // var pan = opt.pan || {};
      // if (pan.interactive && touches.length === 1) {
      //     touch.process_pan({ left: touches[0].pageX, top: touches[0].pageY }, tmout)
      // }

      // // process zoom
      // if (zoom.interactive && touches.length === 2) {
      //     touch.process_zoom(touches, tmout);
      // }           
    }
    ...
    function handleTouchEnd(evt) {
      ...
      if (touches.length === 0 && touch.ttap) {
        ...
        clearTimeout(touch.ttap);
        touch.process_tap(evt, touch.ntap, ktap, touch.tap);
        touch.ttap = setTimeout(function() {
          touch.ttap = touch.ntap = touch.ktap = touch.tap = undefined;
        }, tap_tmout_untouch);
      }
